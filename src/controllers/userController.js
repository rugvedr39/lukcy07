        ...
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                if (result == 0) {
                    ...
                    today = ?,
                    time = ?`;
                        await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, infoBank.sdt,infoBank.tp,infoBank.name_bank, infoBank.email, infoBank.name_user, 0, checkTime, dates]);
